...
function App() {
  ...
  async function applyReview(status, response, timeSavedMinutes) { const saved = currentCase || await saveCase(); if (!saved) return; try { const source = status === 'accepted' ? 'playbook' : 'custom'; const updated = await reviewCase(saved.id, status, response, source, timeSavedMinutes); setCurrentCase(updated); setCases((old) => old.map((item) => item.id === updated.id ? updated : item)); setDashboard(await getDashboard()); setMessage(`Draft ${status}.`); } catch (error) { setMessage(error.message); } }
