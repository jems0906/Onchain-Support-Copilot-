...
function App() {
  ...
  async function diagnose(event) { event.preventDefault(); setLoading(true); setMessage(''); setDraft(null); setRpc(null); setCurrentCase(null); try { const result = await runTriage(form); setTriage(result); const generated = await getDraft(form, result); setDraft(generated); } catch (error) { setMessage(error.message); } finally { setLoading(false); } }
  ...
  async function applyReview(status) { const saved = currentCase || await saveCase(); if (!saved) return; try { const updated = await reviewCase(saved.id, status, draft?.draft); setCurrentCase(updated); setCases((old) => old.map((item) => item.id === updated.id ? updated : item)); setDashboard(await getDashboard()); setMessage(`Draft ${status}.`); } catch (error) { setMessage(error.message); } }
